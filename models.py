...
from sqlalchemy import String, Text, Integer, Float, ForeignKey, Enum, DateTime, func, UniqueConstraint, JSON
...
class Bill(Base):
    ...
    # Congress.gov URL for the most recent plain-text version of the bill.
    # Stored so the UI can fetch and display full bill text on demand without us storing it.
    bill_text_url: Mapped[str | None] = mapped_column(String(500))
    ...
    # Bullet-point list of every concrete impact of a Yea vote, ordered broadest to narrowest.
    # Derived from ai_summary by describe_bills.py. Used on senator record pages.
    yea_impact: Mapped[str | None] = mapped_column(Text)
